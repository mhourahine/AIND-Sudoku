...
#define up boxes, units and peers
boxes = cross(rows, cols)
row_units = [cross(r, cols) for r in rows]
column_units = [cross(rows, c) for c in cols]
square_units = [cross(rs, cs) for rs in ('ABC','DEF','GHI') for cs in ('123','456','789')]
diagonal_units = [[a+b for (a,b) in zip(rows,cols)], [a+b for (a,b) in zip(rows[::-1],cols)]] 
...
def naked_twins(values):
    ...
    for unit in column_units+row_units:
        unit_values = get_values(values, unit)
        naked_twin_boxes = [b for n,b in enumerate(unit) if (len(values[b]) == 2) and (values[b] in unit_values[:n]+unit_values[n+1:])]

        # Eliminate the naked twins as possibilities for their peers
        for nt in naked_twin_boxes:         #each box with a naked twin
            for peer in unit:               #each box that is a peer to this naked twin
                for d in values[nt]:        #each digit in the naked twin
                    if values[peer] != values[nt]:
